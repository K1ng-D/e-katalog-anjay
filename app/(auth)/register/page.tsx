...
export default function RegisterPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (displayName) await updateProfile(user, { displayName: displayName.trim() });
  ...
  return (
    <main className="relative h-screen w-full overflow-hidden bg-gradient-to-b from-white to-slate-50">
      {/* soft blobs */}
      <div className="pointer-events-none absolute -top-28 left-1/2 h-[440px] w-[440px] -translate-x-1/2 rounded-full bg-sky-200/40 blur-3xl" />
            ...
            <div className="px-6 py-7 sm:px-8 sm:py-9">
              ...
              <div className="mb-6 flex items-center justify-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">Buat akun baru e‑Kataloh</p>
                </div>
              </div>
              ...
              <form onSubmit={onSubmit} className="space-y-4">
                ...
                <label className="block">
                  ...
                  <div className="relative flex items-center rounded-xl border border-slate-200 bg-white focus-within:ring-2 focus-within:ring-slate-400">
                    <span className="pl-3 text-slate-400"><FiUser className="h-4 w-4" /></span>
                    <input
                      required
                      value={displayName}
                ...
                <label className="block">
                  ...
                  {pwWeak && (
                    <p className="mt-1 text-xs text-red-600">Password minimal 6 karakter.</p>
                  )}
                </label>
                ...
                <label className="block">
                  ...
                  <div className="relative flex items-center rounded-xl border border-slate-200 bg-white focus-within:ring-2 focus-within:ring-slate-400">
                    ...
                    >
                      {showPw2 ? <FiEyeOff className="h-4 w-4" /> : <FiEye className="h-4 w-4" />}
                    </button>
                  </div>
                  {mismatch && (
                ...
                <label className="flex select-none items-start gap-3 text-sm text-slate-600">
                  ...
                  />
                  <span>
                    Saya setuju dengan <a href="/terms" className="font-medium text-slate-900 hover:underline">Ketentuan Layanan</a> dan
                    <span> </span>
                    <a href="/privacy" className="font-medium text-slate-900 hover:underline">Kebijakan Privasi</a>.
                  </span>
                </label>
                ...
                <button
                  type="submit"
                  disabled={loading || pwWeak || mismatch}
              ...
              <div className="mt-4 flex items-center justify-between text-sm">
                <a href="/login" className="text-slate-500 hover:text-slate-700">Sudah punya akun? Login</a>
                <div className="inline-flex items-center gap-1 text-slate-500">
                  <FiCheck className="h-4 w-4" />
                  <span>Role default: user</span>
                </div>
              </div>
            </div>
          </div>

          <p className="mt-4 text-center text-xs text-slate-500">Data disimpan di Firestore • Timestamp server</p>
        </motion.div>
      </div>
    </main>
  );
