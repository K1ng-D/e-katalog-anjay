              ...
              <img
                src={
                  (typeof food.images[0] === "string"
                    ? (food.images[0] as string)
                    : (food.images[0] as ImageItem)?.url) ?? undefined
                }
                alt={food.name}
...
export default function MakananPage() {
  ...
  const allCategories = Array.from(
    new Set(
      items.map((i) => (i.foodCategory?.trim() || "lainnya").toLowerCase())
    )
  ...
  const filtered = items.filter((it) => {
    const catOk =
      category === "semua" ||
      (it.foodCategory || "lainnya").toLowerCase() === category;
