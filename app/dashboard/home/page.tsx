...
export default function HomePage() {
  ...
  const prefTokens = useMemo(() => {
    ...
    const merged = Array.from(
      new Set([...fromProfile, ...fromSession].filter(Boolean))
    );
    return merged;
  }, [profile?.preferences]);
  ...
  const recoFoodIds = useMemo(() => {
    ...
    const docs = allFoods.map((f) => ({
      id: f.id,
      text: [f.name, f.description, f.foodCategory]
        .filter(Boolean)
        .join(" ")
        .toLowerCase(),
    }));
    ...
    <ProtectedRoute requireSurvey={false}>
      <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
            ...
            <div className="grid md:grid-cols-1 gap-10 items-center">
              ...
              >
                <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-4 leading-tight">
                  Temukan Kebutuhan{" "}
                  <span className="text-yellow-300">Terbaik</span> Anda
                </h1>
                <p className="text-xl md:text-2xl opacity-90 mb-8">
                  E-Katalog dengan rekomendasi personal untuk produk dan makanan
                  favorit Anda
                </p>
              </motion.div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {/* Rekomendasi */}
          {prefTokens.length > 0 &&
            (recoProducts.length > 0 || recoFoods.length > 0) && (
              <motion.div
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {recoProducts.map((p) => (
                          <Card
                            ...
                            img={normalizeImages(p.images)[0]?.url}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        {recoFoods.map((f) => (
                          <Card
                            ...
                            img={normalizeImages(f.images)[0]?.url}
                            overline={(
                              f.foodCategory || "lainnya"
                            ).toUpperCase()}
                            title={f.name}
        ...
        <div className="bg-gradient-to-r from-blue-600 to-purple-700 text-white py-16 mt-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <p className="text-xl opacity-90 mb-8 max-w-3xl mx-auto">
              Jelajahi katalog lengkap kami untuk menemukan lebih banyak produk
              dan makanan menarik.
            </p>
        ...
        <footer className="bg-gray-900 text-white py-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div>
                ...
                <p className="text-gray-400">
                  Temukan produk dan makanan terbaik dengan rekomendasi personal
            ...
            <div className="border-t border-gray-800 mt-8 pt-8 text-center text-gray-400">
              <p>
                © {new Date().getFullYear()} LADUNIMART. All rights reserved.
              </p>
            </div>
          </div>
        </footer>
